...
ci-package:
	...
	@if [ "$(PKG_EXT)" = "rpm" ]; then \
		$(MAKE) -C driver package-rpm || { echo "driver package-rpm failed"; exit 1; }; \
		$(MAKE) -C tools package-rpm || { echo "tools package-rpm failed"; exit 1; }; \
	elif [ "$(PKG_EXT)" = "deb" ]; then \
		$(MAKE) -C driver package-deb || { echo " driver package-deb failed"; exit 1; }; \
		$(MAKE) -C tools package-deb || { echo "tools package-deb failed"; exit 1; }; \
	else \
