...
/** Битовая маска для номера ioctl cmd :
 * 31-30 -- 00 <=> _IO; 10 <=> _IOR; 01 <=> _IOW; 11 <=> _IOWR 
 * 29-16 -- размер передаваемых аргументов
 * 15- 8 -- ascii символ, соответствующий драйверу ( в нашем случае 'a' = 0x61 )
 *  7- 0 -- номер функции 
 * **/
#define IOCTL_CMD_MASK 0xc000ffff
...
struct acle_login_request {
	...
	compat_caddr_t userno;
    /** Флаги аутнетификации */
	compat_caddr_t user_flags;
};

/** Стуктура данных получения случайных чисел для совместимости x32*/
...
/** Стуктура данных получения случайных чисел в случае x64*/
struct acle_rng_request_x64 {
    /** Колиичесвто байт для чтения ДСЧ */
	compat_size_t count;
    /** Буфер для полчаемых случайных байт из аппартного ДСЧ*/
	char *buf;
...
/** Кусок данных для записи во flash-память в случае x64*/
struct acle_data_chunk_x64 {
	/** Смещение в байтах */
	uint64_t offset;
	/** Количество данных в байтах*/
	uint64_t count;
...
/** Данные аутнетификации передаваемы в login x64*/
struct acle_login_request_x64 {
    /** Свертка от пароля и идентификатора*/
  uint8_t hash[32]; 
    /** Номер аутентифицируемого пользователя */
  uint32_t *userno;
...
#define ACLE_GET_BOARD_COUNT	_IOR(ACLE_CMD_MAGIC, 4, compat_caddr_t )
#define ACLE_CMD_LOGOUT		_IO(ACLE_CMD_MAGIC, 5)
#define ACLE_CMD_GET_RANDOM_BYTES	_IOWR(ACLE_CMD_MAGIC, 6, compat_caddr_t )
#define ACLE_CMD_GET_STATE	_IOR(ACLE_CMD_MAGIC, 7, compat_caddr_t )
#define ACLE_CMD_WRITE_TO_BUF0	_IOW(ACLE_CMD_MAGIC, 8, struct acle_data_chunk)
#define ACLE_CMD_READ_FROM_BUF0 _IOR(ACLE_CMD_MAGIC, 9, compat_caddr_t )
#define ACLE_CMD_TM_PRESENT _IOR(ACLE_CMD_MAGIC, 10, compat_caddr_t )
#define ACLE_CMD_TM_GET_ID _IOR(ACLE_CMD_MAGIC, 11, compat_caddr_t )
#define ACLE_CMD_TM_READ_PAGE _IOR(ACLE_CMD_MAGIC, 12, compat_caddr_t )
#define ACLE_CMD_TM_WRITE_PAGE _IOW(ACLE_CMD_MAGIC, 13, compat_caddr_t )
#define ACLE_DISABLE_WATCHDOG_TIMER _IOW(ACLE_CMD_MAGIC, 14, compat_caddr_t )
#define ACLE_CMD_SET_RTC _IOW(ACLE_CMD_MAGIC, 15, struct acle_rtc_time)
#define ACLE_CMD_GET_RTC _IOR(ACLE_CMD_MAGIC, 16, compat_caddr_t )
#define ACLE_CMD_SD_READ _IOR(ACLE_CMD_MAGIC, 17, compat_caddr_t )
#define ACLE_CMD_SD_WRITE _IOW(ACLE_CMD_MAGIC, 18,compat_caddr_t )
#define ACLE_CMD_SET_RANDOM _IOW(ACLE_CMD_MAGIC, 19, int)
#define ACLE_CMD_SET_SDC _IOW(ACLE_CMD_MAGIC, 20, int)

#define ACLE_CMD_GET_STATE_KERNEL _IOR(ACLE_CMD_MAGIC, 21, compat_caddr_t)
